...
average  t  = ((/100) . fromIntegral . round . (*100)) $ (sumTree t) / (fromIntegral (size t))

sumLeaves :: (Num a) => BTree a -> a
sumLeaves (Node value Nil Nil) = value
sumLeaves (Node _ left Nil) = sumLeaves left
sumLeaves (Node _ Nil right) = sumLeaves right
sumLeaves (Node _ left right ) = sumLeaves left + sumLeaves right 


areEqual :: (Eq a ) => BTree a -> BTree a -> Bool
areEqual xs ys = traverseDFS xs == traverseDFS ys
